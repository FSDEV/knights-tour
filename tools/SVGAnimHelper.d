...
void main(char args[][]) {
	//foreach(int idx, char[] foo; args) // diagnostic for testing purposes
	//	Stdout.format("{} : {}\n", idx, foo);
	
	bool file_mode=false;
	bool force=false;
	char[] file;
	char[] output_file="svg.svg";
	char[] path;
	int board_x=8;
	int board_y=8;
	...
	parser.bindPosix("file", (char[] value){
			Stdout.format("file={}", value).newline;
			file=value;
	});
	...
	if(file_mode){
		...
		foreach(char[] line; lines){
			...
			else if( contains(temp, "found") /+line[0..4]=="found"+/){
				temp=""; int xnoty=0;
				foreach(char c; line){
					if(c>'0'&&c<'9')
						temp~=c;
		...
		path="0,1 2,0 1,2 0,0 2,1 0,2 1,0 3,1 5,0 7,1 6,3 7,5 6,7 4,6 2,7 0,6 1,4 3,5 1,6 3,7 5,6 7,7 6,5 5,7 7,6 6,4 7,2 6,0 4,1 6,2 7,0 5,1 3,0 1,1 0,3 1,5 0,7 2,6 4,7 6,6 7,4 5,5 3,6 1,7 0,5 1,3 3,4 5,3 6,1 4,0 3,2 4,4 2,5 0,4 2,3 4,2 5,4 7,3 5,2 3,3 4,5 2,4 4,3 2,2";
		
		char len[][]=txtUtil.split(path, " ");
		
		char[] foo;
		
		FileConduit fcond2;
		scope(exit)fcond2.close();
		
		for(int i=2; i!=len.length; i++) {
			foo="";
			foreach(char[] bar; len[0 .. i])
			foo~=bar~" ";
			fcond2=new FileConduit("svg"~Integer.toString(i)~".svg", FileConduit.WriteCreate);
			fcond2.output.write(make_svg(foo[0..$-1], board_x, board_y, 10));
			fcond2.close();
		}
		
		Stdout(make_svg(path, board_x, board_y, 10));
